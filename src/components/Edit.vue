...
export default {
  ...
  methods: {
    addMilk () {
      ...
      if (data.length > 0) {
        ...
        this.$http.post('http://localhost:3003/milk', params).then((data) => {
          ...
          this.$router.push({name: 'Edit', params: {id: data.body.date}})
          this.disabled = false
        }, () => {
          console.log('error')
    ...
    getMilk () {
      this.$http.get('http://localhost:3003/milk', {id: 0}).then((data) => {
        console.log(data.body)
        this.milk_date = data.body[0].date
        this.milk_yield = data.body[0].milk_yield
      }, () => {
        console.log('error')
      })
    },
